...
└───────────────────────┘
```
1. First the lexer parses the source code and converts the characters to tokens. Tokens are the smallest usable pieces of information the compiler needs to compile the program. The reason we use tokens is because it's much easier to express logic further down in the pipeline and its alot faster to operate on tokens than it is on strings.
